...
/****************************************************************************/
{
	...
	else
	{
		...
		case IDLE:  AbiJtagIdle();
					jam_jtag_state = IDLE;
					break;

		case DRPAUSE: switch (jam_jtag_state) {
							case IRPAUSE:  
									AbiJtagIdle();
							case RESET:
							case IDLE:		
		...
		case IRPAUSE: switch (jam_jtag_state) {
							case DRPAUSE:
									AbiJtagIdle();
							case RESET:
							case IDLE:
		...
		default:
				// FS2 Can't move to an unstable state
				break;
		}
	}
#else
...
{	int i = 0;
	...
	switch (start_state) {
	case IRPAUSE:  AbiJtagIdle();
	case RESET:
	case IDLE:     AbiJtagDrEnter();
	}

	if (tdo == NULL) {
		AbiJtagShift(tdi, count);
	} else {
		while (count >= 256*8) {
			AbiJtagShift(tdi, 256*8);
			AbiJtagRead(tdo, 256*8);
			tdo += 256;
			tdi += 256;
			count -= 256*8;
		}
...
)
{
	...
	switch (start_state) {
	case DRPAUSE:  h=AbiJtagIdle();
	case RESET:
	case IDLE:     h=AbiJtagIrEnter();
	}

    if (h != GOOD)
		printf(AbiGetErrorText(h));
